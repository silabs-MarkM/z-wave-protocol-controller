...
3. **MkDocs Publishing Workflow**
   ...
   python3 scripts/mqtt_asyncapi/assemble_and_render.py \
     --root . \
     --cc-root build/command_class_docs \
...
4. **MQTT API Documentation**
   - MQTT topics and payloads are defined in AsyncAPI: a root spec in [`components/mqtt_api/asyncapi/asyncapi.yaml`](components/mqtt_api/asyncapi/asyncapi.yaml), per-component YAML under `components/<module>/asyncapi/`, and command-class fragments under `components/command_classes/<command_class>/asyncapi/` generated from `zwave.xml`
